...
type Conversation = {
  ...
  listingTitle: string;
  otherName: string;
  // How many messages in this thread are addressed to me and still
  // unread. Counted from the same rows the list is already built from,
...
export default function MessagesScreen() {
  ...
  //
  // This screen used to load its conversations exactly once. Open a
  // chat — which calls markConversationRead() — press back, and the
  // list still showed the thread as unread, because nothing had asked
  // the database again. expo-router keeps the screen mounted underneath
  // the pushed route, so the mount effect never re-ran; the only way to
  // refresh was to kill the app.
  //
  // Harmless while the rows showed no unread state at all. The moment
  // they carry a count, a badge that survives reading the messages is
  // the whole feature failing in the most visible way possible.
            ...
            >
              ...
              <View style={styles.convoBody}>
                <View style={styles.convoTopRow}>
                  <Text
                    style={[styles.convoName, c.unread > 0 && styles.convoNameUnread]}
                    numberOfLines={1}
                  >
                    {c.otherName || 'ImbizoHub user'}
                  </Text>
                  <Text style={[styles.convoTime, c.unread > 0 && styles.convoTimeUnread]}>
                    {formatTime(c.lastAt)}
                  </Text>
                </View>
                ...
                <View style={styles.convoBottomRow}>
                  ...
                  {/* The count, the way every messaging app people
                      already use shows it. Capped at 99+ so one runaway
                      thread cannot stretch the row. */}
                  {c.unread > 0 ? (
                    <View
...
const styles = StyleSheet.create({
  ...
  // UNREAD STATE. Weight and brightness carry it as well as the badge:
  // a count alone is easy to miss in a long list, and colour alone is
  // no use to anyone who cannot distinguish it.
